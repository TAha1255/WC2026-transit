...
function PaymentModal({ lang, onClose }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.92)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
      <div style={{background:'#0a1224',border:'1px solid rgba(245,158,11,0.4)',borderRadius:'16px',padding:'28px',maxWidth:'400px',width:'100%'}}>
        <div style={{textAlign:'center',marginBottom:'16px'}}>
          <div style={{fontSize:'36px'}}>🏆</div>
          <h2 style={{color:'#f8fafc',fontSize:'18px',fontWeight:700,margin:'8px 0 4px'}}>
            {lang==='ar'?'احصل على الوصول الكامل':lang==='es'?'Obtén Acceso Completo':'Get Full Access'}
          </h2>
        ...
        {/* PayPal */}
        <button
          onClick={()=>{ window.location.href='https://www.paypal.com/paypalme/tahafaradj/5' }}
          style={{width:'100%',padding:'13px',background:'#003087',border:'none',borderRadius:'8px',color:'white',fontSize:'14px',fontWeight:700,cursor:'pointer',marginBottom:'10px',display:'block'}}>
          💳 Pay with PayPal
        </button>

        <div style={{textAlign:'center',color:'#334155',fontSize:'11px',fontFamily:'monospace',margin:'10px 0'}}>── OR CRYPTO ──</div>

        {/* Coin selector */}
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px',marginBottom:'12px'}}>
          {Object.entries(WALLETS).map(([c,w])=>(
            <button key={c} onClick={()=>setCoin(c)}
              style={{padding:'8px 4px',borderRadius:'6px',border:`1px solid ${coin===c?w.color+'88':'rgba(255,255,255,0.07)'}`,background:coin===c?`${w.color}15`:'rgba(255,255,255,0.02)',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:'3px'}}>
              <span style={{fontSize:'16px'}}>{w.icon}</span>
              <span style={{color:coin===c?w.color:'#475569',fontFamily:'monospace',fontSize:'9px',fontWeight:coin===c?700:400}}>{c}</span>
            </button>
          ))}
        </div>

        {/* Network */}
        <div style={{background:`${wallet.color}12`,border:`1px solid ${wallet.color}33`,borderRadius:'6px',padding:'6px 10px',marginBottom:'8px',display:'flex',justifyContent:'space-between'}}>
          <span style={{color:'#64748b',fontSize:'10px',fontFamily:'monospace'}}>Network</span>
          <span style={{color:wallet.color,fontSize:'10px',fontFamily:'monospace',fontWeight:700}}>{wallet.network}</span>
        </div>
        ...
        <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'8px',padding:'10px',marginBottom:'8px',display:'flex',gap:'8px',alignItems:'center'}}>
          <span style={{color:'#94a3b8',fontFamily:'monospace',fontSize:'9px',flex:1,wordBreak:'break-all',lineHeight:1.5}}>{wallet.address}</span>
          <button onClick={copyAddr}
            style={{padding:'5px 8px',borderRadius:'5px',border:`1px solid ${wallet.color}55`,background:`${wallet.color}15`,color:wallet.color,fontFamily:'monospace',fontSize:'9px',cursor:'pointer',flexShrink:0}}>
            {copied?'✅':lang==='ar'?'نسخ':'Copy'}
          </button>
        </div>

        {/* Warning */}
        <div style={{background:'rgba(239,68,68,0.06)',border:'1px solid rgba(239,68,68,0.2)',borderRadius:'6px',padding:'7px 10px',marginBottom:'12px'}}>
          <span style={{color:'#f87171',fontSize:'11px'}}>⚠️ {lang==='ar'?'أرسل إثبات التحويل على واتساب بعد الدفع':'Send payment proof on WhatsApp after transfer'}</span>
        </div>

        {/* WhatsApp */}
        <button
          onClick={()=>{ window.location.href=`https://wa.me/96565724949?text=${encodeURIComponent('WC2026 Pro Payment - '+coin+' $5')}` }}
          style={{width:'100%',padding:'12px',background:'#25d366',border:'none',borderRadius:'8px',color:'white',fontSize:'13px',fontWeight:700,cursor:'pointer',marginBottom:'8px'}}>
...
const WA  = '96565724949'
const PP  = 'https://www.paypal.com/paypalme/tahafaradj/5'
...
function PaymentModal({ lang, onClose }) {
  ...
  const copy = () => { navigator.clipboard.writeText(w.address).catch(()=>{}); setCopied(true); setTimeout(()=>setCopied(false),2000) }
  const paypal = () => { window.open(PP,'_blank') }
  const whatsapp = () => { window.open(`https://wa.me/${WA}?text=${encodeURIComponent(`WC2026 Pro $5\nCoin: ${coin}\nNet: ${w.network}`)}`,'_blank') }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.92)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}
      ...
      <div style={{background:'#0a1224',border:'1px solid rgba(245,158,11,0.4)',borderRadius:'16px',padding:'24px',maxWidth:'400px',width:'100%'}}>

        <div style={{textAlign:'center',marginBottom:'16px'}}>
          <div style={{fontSize:'36px'}}>🏟️</div>
          <h2 style={{color:'#f8fafc',fontSize:'18px',fontWeight:700,margin:'8px 0 4px'}}>
            {lang==='ar'?'ترقية للـ Pro':lang==='es'?'Upgrade a Pro':'Upgrade to Pro'} 🏆
          </h2>
          <div style={{background:'rgba(245,158,11,0.1)',border:'1px solid rgba(245,158,11,0.3)',borderRadius:'8px',padding:'10px',margin:'12px 0'}}>
            <div style={{color:'#f59e0b',fontSize:'28px',fontWeight:900}}>$5</div>
            <div style={{color:'#64748b',fontSize:'11px'}}>
              {lang==='ar'?'وصول كامل لكل البطولة — مرة واحدة':lang==='es'?'Acceso completo — pago único':'Full tournament access — one time'}
            </div>
          </div>
        </div>

        {/* PayPal */}
        <div onClick={paypal} style={{background:'#0070ba',borderRadius:'8px',padding:'12px',textAlign:'center',cursor:'pointer',marginBottom:'10px',color:'white',fontWeight:700,fontSize:'14px'}}>
          💳 {lang==='ar'?'ادفع عبر PayPal':lang==='es'?'Pagar con PayPal':'Pay with PayPal'}
        </div>

        <div style={{textAlign:'center',color:'#334155',fontSize:'11px',fontFamily:'monospace',margin:'8px 0'}}>— OR CRYPTO —</div>

        {/* Coin select */}
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px',marginBottom:'10px'}}>
          {Object.entries(WALLETS).map(([c,v])=>(
            <div key={c} onClick={()=>setCoin(c)}
              style={{padding:'8px',borderRadius:'8px',border:`1px solid ${coin===c?v.color+'88':'rgba(255,255,255,0.07)'}`,background:coin===c?`${v.color}15`:'rgba(255,255,255,0.02)',cursor:'pointer',textAlign:'center'}}>
              <div style={{fontSize:'16px'}}>{v.icon}</div>
              <div style={{color:coin===c?v.color:'#475569',fontFamily:'monospace',fontSize:'9px',fontWeight:coin===c?700:400}}>{c}</div>
            </div>
          ))}
        </div>
        ...
        {/* Address */}
        <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'8px',padding:'8px 10px',marginBottom:'8px',display:'flex',gap:'8px',alignItems:'center'}}>
          <span style={{color:'#94a3b8',fontFamily:'monospace',fontSize:'9px',flex:1,wordBreak:'break-all',lineHeight:1.6}}>{w.address}</span>
          <div onClick={copy} style={{padding:'5px 8px',borderRadius:'5px',border:`1px solid ${w.color}44`,background:`${w.color}12`,color:w.color,fontFamily:'monospace',fontSize:'9px',cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>
            {copied?'✅':lang==='ar'?'نسخ':'Copy'}
          </div>
        </div>

        {/* Steps */}
        <div style={{background:'rgba(255,255,255,0.02)',borderRadius:'6px',padding:'8px 12px',marginBottom:'8px'}}>
          <div style={{color:'#64748b',fontSize:'11px',marginBottom:'3px'}}>
            {lang==='ar'?'١. انسخ العنوان ٢. حوّل $5 ٣. أرسل إثبات على واتساب':'1. Copy address 2. Send $5 3. Send proof on WhatsApp'}
          </div>
        </div>

        {/* Warning */}
        <div style={{background:'rgba(239,68,68,0.06)',border:'1px solid rgba(239,68,68,0.2)',borderRadius:'6px',padding:'6px 10px',marginBottom:'12px'}}>
          <span style={{color:'#f87171',fontSize:'10px'}}>⚠️ {lang==='ar'?'تأكد من الشبكة الصحيحة قبل التحويل':'Use correct network before sending'}</span>
        </div>

        {/* WhatsApp */}
        <div onClick={whatsapp} style={{background:'#25d366',borderRadius:'8px',padding:'12px',textAlign:'center',cursor:'pointer',marginBottom:'8px',color:'white',fontWeight:700,fontSize:'13px'}}>
          📱 {lang==='ar'?'أرسل إثبات الدفع عبر واتساب':lang==='es'?'Enviar comprobante por WhatsApp':'Send Payment Proof on WhatsApp'}
        </div>

        {/* Close */}
        <div onClick={onClose} style={{textAlign:'center',padding:'10px',color:'#475569',fontSize:'12px',cursor:'pointer',borderRadius:'8px',border:'1px solid rgba(255,255,255,0.07)'}}>
          {lang==='ar'?'رجوع':lang==='es'?'Volver':'Go Back'}
        </div>
...
function UsageBar({used,lang}) {
  const r = FREE_LIMIT-used
  const msg = lang==='ar'?`${r} خطة مجانية متبقية`:lang==='es'?`${r} planes gratuitos restantes`:`${r} free plan${r!==1?'s':''} remaining`
  return (
    <div style={{background:'rgba(245,158,11,0.06)',border:'1px solid rgba(245,158,11,0.15)',borderRadius:'8px',padding:'10px 14px',marginBottom:'14px',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'8px',flexWrap:'wrap'}}>
      <span style={{color:'#94a3b8',fontSize:'12px'}}>{msg}</span>
...
export default function Home() {
  ...
  const venue=VENUES[venueId], match=venue.matches[matchIdx], alsdt=calcALSDT(match.date,venue), strats=STRATS(lang), steps=result?.strategies?.[activeStrat]?.steps||[]
  ...
  return (
    <div style={{maxWidth:'900px',margin:'0 auto',padding:'18px 14px',direction:dir}}>
      ...
      {showPaywall && <PaymentModal lang={lang} onClose={()=>setShowPaywall(false)}/>}
      ...
      <div style={{display:'flex',gap:'6px',marginBottom:'16px'}}>
        {Object.entries(LANGUAGES).map(([code,l])=>(
          <div key={code} onClick={()=>setLang(code)}
            style={{padding:'6px 12px',borderRadius:'6px',border:`1px solid ${lang===code?'#f59e0b55':'rgba(255,255,255,0.08)'}`,background:lang===code?'rgba(245,158,11,0.12)':'transparent',color:lang===code?'#f59e0b':'#475569',fontFamily:'monospace',fontSize:'11px',cursor:'pointer',display:'flex',alignItems:'center',gap:'5px',userSelect:'none'}}>
            {l.flag} {l.label}
          </div>
        ))}
      </div>
      ...
      <div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginBottom:'14px'}}>
        {Object.values(VENUES).map(v=>(
          <div key={v.id} onClick={()=>setVenueId(v.id)}
            style={{padding:'8px 12px',borderRadius:'8px',border:`1px solid ${venueId===v.id?'#f59e0b55':'rgba(255,255,255,0.06)'}`,background:venueId===v.id?'rgba(245,158,11,0.1)':'rgba(255,255,255,0.02)',color:venueId===v.id?'#f59e0b':'#334155',fontSize:'11px',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:'2px',userSelect:'none'}}>
      ...
      <div style={{background:'rgba(10,18,36,0.9)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:'12px',padding:'18px',marginBottom:'14px'}}>
        ...
        <div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginBottom:'14px'}}>
          {venue.suggested.map(s=>(
            <div key={s} onClick={()=>setOrigin(s)}
              style={{padding:'4px 9px',borderRadius:'4px',border:`1px solid ${origin===s?'#f59e0b44':'rgba(255,255,255,0.06)'}`,background:origin===s?'rgba(245,158,11,0.08)':'transparent',color:origin===s?'#fcd34d':'#334155',fontFamily:'monospace',fontSize:'9px',cursor:'pointer',userSelect:'none'}}>
              {s.split(',')[0]}
            </div>
          ))}
        </div>
        ...
        <div onClick={generate}
          style={{width:'100%',padding:'13px',background:loading?'rgba(245,158,11,0.06)':'rgba(245,158,11,0.12)',border:`1px solid ${loading?'rgba(245,158,11,0.1)':'rgba(245,158,11,0.4)'}`,borderRadius:'8px',color:loading?'#78350f':'#f59e0b',fontSize:'14px',fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',userSelect:'none',boxSizing:'border-box'}}>
        ...
        <>
          <div style={{display:'flex',gap:'6px',marginBottom:'12px'}}>
            {Object.entries(strats).map(([k,cfg])=>(
              <div key={k} onClick={()=>setActiveStrat(k)}
                style={{flex:1,padding:'9px',borderRadius:'6px',border:`1px solid ${activeStrat===k?cfg.color+'55':'rgba(255,255,255,0.06)'}`,background:activeStrat===k?`${cfg.color}10`:'transparent',color:activeStrat===k?cfg.color:'#334155',fontSize:'11px',cursor:'pointer',fontWeight:activeStrat===k?700:400,textAlign:'center',userSelect:'none'}}>
                {cfg.label}
              </div>
            ))}
          </div>
          {(()=>{
            const s=result.strategies[activeStrat], cfg=strats[activeStrat]
            if(!s) return null
            return (
              <div style={{background:'rgba(10,18,36,0.9)',border:`1px solid ${cfg.color}28`,borderRadius:'10px',padding:'16px',marginBottom:'12px'}}>
                ...
                {(s.steps||[]).map((st,i)=>{
                  const mode=(st.mode||'WALK').toUpperCase(), col=MODE_COLORS[mode]||'#f59e0b'
                  return (
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:'8px',marginBottom:'14px'}}>
            <div onClick={()=>{}} style={{padding:'10px',background:'rgba(16,185,129,0.07)',border:'1px solid rgba(16,185,129,0.25)',borderRadius:'8px',color:'#10b981',fontSize:'12px',cursor:'pointer',textAlign:'center',userSelect:'none'}}>📅 {t(lang,'export_cal')}</div>
            <div onClick={copyPlan} style={{padding:'10px',background:'rgba(167,139,250,0.07)',border:'1px solid rgba(167,139,250,0.25)',borderRadius:'8px',color:'#a78bfa',fontSize:'12px',cursor:'pointer',textAlign:'center',userSelect:'none'}}>{copied?t(lang,'copied'):`📋 ${t(lang,'copy_plan')}`}</div>
            <div onClick={()=>window.print()} style={{padding:'10px',background:'rgba(245,158,11,0.07)',border:'1px solid rgba(245,158,11,0.25)',borderRadius:'8px',color:'#f59e0b',fontSize:'12px',cursor:'pointer',textAlign:'center',userSelect:'none'}}>🖨️ {t(lang,'print')}</div>
          </div>
        </>
      )}
      ...
      {usage>=FREE_LIMIT&&(
        <div onClick={()=>setShowPaywall(true)}
          ...
          💳 {lang==='ar'?'احصل على الوصول الكامل — $5':lang==='es'?'Acceso Completo — $5':'Get Full Access — $5'}
        </div>
      )}

      <div style={{marginTop:'60px',textAlign:'center',color:'#0f172a',fontFamily:'monospace',fontSize:'8px'}}>
