...
function getUsage() { try { return parseInt(localStorage.getItem(STORAGE_KEY) || '0') } catch { return 0 } }
function incrementUsage() { try { localStorage.setItem(STORAGE_KEY, String(getUsage() + 1)) } catch {} }

function calcALSDT(kickoffISO, venue) {
  if (!kickoffISO || !venue) return null
  const kickoff = new Date(kickoffISO)
  return new Date(kickoff.getTime() - (30 + venue.security_wait + 15 + 35 + 10) * 60000)
}
...
export default function Home() {
  ...
  return (
    <div style={{maxWidth:'900px',margin:'0 auto',padding:'18px 14px',direction:dir}}>
      ...
      {/* PAYWALL */}
      {showPaywall && <CryptoPayment lang={lang} onClose={()=>setShowPaywall(false)}/>}
      ...
      <div style={{background:'rgba(10,18,36,0.9)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:'12px',padding:'18px',marginBottom:'14px'}}>
        ...
        <button onClick={generate} disabled={loading}
          style={{width:'100%',padding:'13px',background:loading?'rgba(245,158,11,0.06)':'rgba(245,158,11,0.12)',border:`1px solid ${loading?'rgba(245,158,11,0.1)':'rgba(245,158,11,0.4)'}`,borderRadius:'8px',color:loading?'#78350f':'#f59e0b',fontSize:'14px',fontWeight:700,cursor:loading?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',marginBottom:'8px'}}>
          {loading
